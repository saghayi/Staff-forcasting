...
def extract_features(date: datetime.date) -> Dict:
    return dict(year=date.year, month=date.month, day=date.day, weekday=date.weekday())


def extract_features_from_series(date_series: Type[pd.Series]) -> Dict[str, List]:
    weekday = list(map(lambda x: x.weekday(), date_series))
    day = list(map(lambda x: x.day, date_series))
    month = list(map(lambda x: x.month, date_series))
    year = list(map(lambda x: x.year, date_series))
    return dict(year=year, month=month, day=day, weekday=weekday)


# from  https://github.com/rodrigo-arenas/pyworkforce/blob/main/pyworkforce/queuing/erlang.py
class ErlangC:
